namespace AssociationRegistry.KboMutations.MutationFileLambda.Telemetry;
...
public class TelemetryManager : IDisposable
{
    ...
    public async Task FlushAsync(ILambdaContext context)
    {
        ...
        {
            _logger.LogLine($"Error during OpenTelemetry flush: {ex.Message}");
        }
    }
    ...
    public void Dispose()
    {
        ...
        {
            _logger.LogLine($"Error disposing OpenTelemetry setup: {ex.Message}");
        }
    }
}
